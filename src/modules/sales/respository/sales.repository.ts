...
export class SalesRepository {
  ...
  async findSaleByPeriod(start: string, end: string) {
    return await this.dataSource.query(`
        ...
        FROM sales s
            LEFT JOIN customers c on c.id = s.customer_id 
            LEFT JOIN users     u on u.id = s.user_id
        WHERE s.date >= $1 and s.date <= $2`, [start, end]
    );
  }

  async create(data: { date: string; total_price:number; customer_id: number; user_id: number;  }) {
    const result = await this.dataSource.query(
      `INSERT INTO sales ( "date" , "totalPrice" , "customer_id" , "user_id" ) 
      VALUES ($1, $2, $3, $4) RETURNING "id", "date" , "totalPrice"  as "total_price", "customer_id" , "user_id"`,
  ...
  async update(id: number, data: { date: string; total_price:number; customer_id: number;  }) {
    const result = await this.dataSource.query(
      `UPDATE sales SET date = $1, "totalPrice" = $2, customer_id = $3  
      WHERE id = $4 RETURNING "id", "date", "totalPrice", "customer_id", "user_id"`,
